...
router.post('/', checkEmail,checkUserName, function(req, res, next) {
  ...
  userDetails.save((err,res1)=>{
    if(err) throw err;
    res.render('index', { title: 'Express',msg:'User Successfuly Register' });
  }); 
});
...
router.get('/NewCategory', checkUserLogin,function(req, res, next) {
  var  user= req.session.userName;
  res.render('NewCategory', { title: 'Add New password category' ,msg:user,success:''});
});
...
router.post('/NewCategory',  checkUserLogin,function(req, res, next) {
  ...
  categoryDetail.save((err,res1)=>{
    if(err) throw err;
    res.render('NewCategory', { title: 'All password category' ,msg:user,success:'Category Added Successfully '});
  });
...
router.get('/deleteCate/:id', checkUserLogin,function(req, res, next) {
  ...
  var updateCate=categoryModel.findById(id);
   updateCate.exec((err,data)=>{
     ...
     if(data)
     {
      res.render('updateCate', { title: 'All password Details' ,msg:user,record:data,editMsg:''});
     }
  ...
  getUserPassDetail.exec((err,data)=>{
   ...
   i++;
    ...
    passwordModel.countDocuments({userName:user}).exec((err,count)=>{
      res.render('passwordDetails', {title:'All password Details', passdata:dec,record: data,
        current: page,
...
router.get('/NewDetails', checkUserLogin,function(req, res, next) {
  ...
  getCategory.exec((err,data)=>{
    if(err) throw err;
    res.render('NewDetails', { title: 'All password category',msg:user ,record:data,success:''});
  });
...
router.get('/editPass/:id', checkUserLogin,function(req, res, next) {
  ...
  var editPass=passwordModel.findById(id);
   editPass.exec((err,data1)=>{
     ...
     getCategory.exec((err,data)=>{
      if(err) throw err;
      res.render('editPass', { title: 'All password category',msg:user ,record:data,record1:data1,DcrptPass:dec, success:''});
    });
  });
   });
